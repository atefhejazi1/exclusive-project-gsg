...
const Index = ({ slug, heading, isBestSelling, isFlashSales }) => {
  const filteredProducts = isBestSelling
    ? products.filter((product) => product.bestSelling)
    : products;

  let itemsPerPage = isFlashSales ? 5 : 8;
    ...
    <div className="">
      ...
      <div className="flex flex-col mb-5 max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 gap-6 pt-20 pb-5">
        <div className="flex items-center">
          ...
          <h3 className="ml-5 text-red-700 text-lg font-semibold">{slug}</h3>
        </div>
      ...
      <div
        className={`${
          isFlashSales
            ? "flex  overflow-hidden  max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 gap-6 "
            : "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4  max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 gap-6"
        }`}
      >
        {currentProducts.map((product, index) => (
          <div key={index} className={isFlashSales ? "flex-none w-[22%]" : ""}>
            <Product product={product} isFlashSales={isFlashSales} />
          </div>
        ))}
      </div>
